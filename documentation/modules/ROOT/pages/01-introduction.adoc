...
For containers started back in 1979 with Unix chroot. This Unix system call basically allows the user to change a process and its childrens root directory, this way the main process will believe that is in the root directory, isolating it from other processes in the operating system.

It is been a long ride since processes isolation started in 1979 until today, where many technologies has been key to evolve and achieve today’s containers maturity, the most revelevants could be summarized below:
