...
Basically a simple OCI implementation requires at least to specifications to be used, image-spec and runtime-spec and it would follow the workflow described in the below diagram.

image::oci-implementation.png[]


As it could be seen, to have the same user experience (UX) using different tools (docker, podman, rkt), should have the same result when the container is managed. The diagram shows that image that includes the necessary information to be deployed in a Container, once the image has been downloaded is sent to the runtime to be processed, the filesystem is unpacked and run it at this point.
