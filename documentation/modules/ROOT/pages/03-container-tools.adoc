...
https://github.com/containers/podman[Podman]: is a command line tool that its name does not come from a funny story like buildah, but more like most of the tools, is an abbreviation of ‘Pod Manager’, and as its name points is designed to manage and run containers, images and volumes. The podman’s core is based on a golang library called libpod, which is in charge of all actions that the tool could perform. . 
